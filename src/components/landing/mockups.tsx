          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isActive
                  ? 'border-morrison-300 bg-morrison-50'
                  : isHovered
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className={cn('truncate text-xs font-semibold transition-colors', isHovered ? 'text-slate-900' : 'text-slate-700')}>{r.name}</p>
                <div className="mt-0.5 h-1 overflow-hidden rounded-full bg-slate-200">
                  <motion.div
                    className={cn('h-full rounded-full', r.sc)}
                    initial={{ width: 0 }}
                    animate={{ width: `${r.pct}%`, filter: isHovered ? 'brightness(1.12)' : 'brightness(1)' }}
                    transition={{ duration: 0.8, delay: 0.2 }}
                  />
                </div>
              </div>
              <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold transition-opacity', r.ss, isHovered ? 'opacity-100' : 'opacity-80')}>{r.stage}</span>
              <motion.div animate={{ x: isHovered ? 3 : 0 }} transition={{ duration: 0.15 }}>
                <ChevronRight className={cn('h-3.5 w-3.5 flex-shrink-0 transition-colors', isActive ? 'text-morrison-500' : isHovered ? 'text-slate-500' : 'text-slate-300')} />
              </motion.div>
            </motion.div>
          );
        })}
